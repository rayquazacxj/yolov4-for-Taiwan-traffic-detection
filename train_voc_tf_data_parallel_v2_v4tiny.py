...
from src.Data_voc_v4tiny import Data
from src.YOLO_v4tiny import YOLO
from os import path
...
if config.voc_anchors_v4tiny:
    anchors =  np.asarray(config.voc_anchors_v4tiny).astype(np.float32).reshape([-1, 3, 2])
else:
    anchors = None
...
def backward():
    ...
    data = Data(voc_root_dir, names_file, class_num, batch_size*num_gpus, 
                            anchors, is_tiny=True, size=size)
    ...
    inputs, y1_true, y2_true = iterator.get_next() #y1 = 13、19 y2=26、38
        ...
        for i in range(num_gpus):
            ...
            with tf.device(assign_to_device('/gpu:{}'.format(i), ps_device='/cpu:0')):
                with tf.name_scope('GPU_%d' % i) as scope:  
                    ...
                    feature_y1, feature_y2 = yolo.forward(_inputs, class_num, weight_decay=weight_decay, isTrain=True,reuse=reuse_vars)
    ...
    #                 feature_y1_test, feature_y2_test, feature_y3_test = yolo.forward(_inputs, class_num, weight_decay=weight_decay, isTrain=True,reuse=True)
                    ...
                    loss = Loss().yolo_loss([feature_y1, feature_y2], 
                                                                        [_y1_true, _y2_true], 
                                                                        [anchors[1], anchors[0]], 
